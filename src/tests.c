...
void test1__(gc_state_t state) {
    long_table_t d1 =
        register_gc_data(state, long_table_new(), sizeof(struct LongTable),
                         long_table_free, NULL);
    long_table_add(d1, 1, (void *)1);

    // Currently breaks if these print statements dont happen - literally thats
    // it. Either one can happen and it works. Perhaps due to placement of
    // pointers in registers vs stack? register checking code may not properly
    // work.
    // #ifdef DEBUG
